...
namespace ATHMovil.Purchase.Openers
{
    internal class Opener : IOpener
    {
        ...
        private const string MobileSecure = "mobileSecure";
        ...
        OpenerResult IOpener.OpenATHMovil(PurchaseRequest request, ATHMovilTarget target)
        {
            ...
            string fullPath = target.GetTargetUniversalLink() + GetPathPaymentType(request);
        ...
        /// <returns></returns>
        private string GetPathPaymentType(PurchaseRequest request)
        {
            if (request.Business.IsDummy)
            {
                return PathDummyToken;
            }

            if (request.Purchase.EcommerceId != null && !request.Purchase.EcommerceId.Equals("")) {
                return MobileSecure;
            }
        ...
        internal static string GetTargetUniversalLink(this ATHMovilTarget target)
        {
            switch (target)
            {
                case ATHMovilTarget.DEV:
                    return "https://athmovil-ios-qa.web.app/e-commerce/";
